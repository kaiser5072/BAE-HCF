...
class AE_CF(object):
    ...
    def builder(self, inputs, sides):
        w_init = tf.initializers.truncated_normal(stddev=0.01)
        ...
        pred_top_k, _ = tf.nn.top_k(self.outputs, k=100)
        pref_diff_zero = tf.reduce_mean(tf.square(pred_top_k))
        # pref_diff_ones = tf.reduce_mean(tf.square(self.preds - 1))

        self.loss = tf.add_n([pref_diff_zero])
    ...
    def _BAE_model_fn(self, features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            predictions = {
                ...
                'ratingTrain': tf.sparse.to_dense(features['pref'])
            }
